//
//  UGAssessmentPageViewController.m
//  UrgooApp
//
...
#import "UGTestPageViewController.h"
#import "UGGotoTestDtlViewController.h"
@interface UGAssessmentPageViewController ()

@end

@implementation UGAssessmentPageViewController
...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if (pathArr.count>1) {
        ...
        if ([pathArr[0] isEqualToString:gotoTestDtl]) {
             //[self.navigationController popViewControllerAnimated:YES];
            UGGotoTestDtlViewController *vc = [[UGGotoTestDtlViewController alloc]init];
            vc.urlStr = urlStr;
            [self pushToNextViewController:vc];
            
            
        }else if([pathArr[0] isEqualToString:gotoTestPage]) {
           
                        UGTestPageViewController *vc = [[UGTestPageViewController alloc]init];
                        vc.urlStr = urlStr;
                        [self pushToNextViewController:vc];
