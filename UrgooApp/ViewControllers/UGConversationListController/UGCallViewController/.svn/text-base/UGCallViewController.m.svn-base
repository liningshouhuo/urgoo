...
@interface UGCallViewController ()
{
...
@implementation UGCallViewController
...
#pragma mark - 获取视频的头像和昵称
-(void)getHeadImageView_Nick{
    //头像&昵称的处理
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"token"] = kToken;
    params[@"userHxCodeString"] = _callSession.remoteUsername;
    params[@"termType"] = @"2";
        ...
        if (json) {
            ...
            if (userInfoList.count != 0) {//获取到信息
                ...
                if (headImageUrl.length != 0) {//有头像
                    headImageUrl = [NSString stringWithFormat:@"%@%@",UG_HOST,headImageUrl];
                    [_headerImageView sd_setImageWithURL:[NSURL URLWithString:headImageUrl] placeholderImage:[UIImage imageNamed:@"user"]];
                }else{
                    _headerImageView.image = [UIImage imageNamed:@"EaseUIResource.bundle/user"];
                }
...
- (void)answerAction
{
    ...
    [[ChatHelper shareHelper] answerCall];
...
- (void)hangupAction
{
    ...
    [[ChatHelper shareHelper] hangupCallWithReason:EMCallEndReasonHangup];
