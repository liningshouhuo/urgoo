...
@interface ChatHelper ()<EMCallManagerDelegate>
{
    NSTimer *_callTimer;
}
@end
...
-(void)add_notification:(EMMessage *)message{
    NSNotification *notification =[NSNotification notificationWithName:@"showNotificationWithMessage" object:message userInfo:nil];
    //通过通知中心发送通知
                       ...
                       reason:(EMGroupLeaveReason)aReason
{
    ...
    if (str.length > 0) {
//        TTAlertNoTitle(str);
        [UGAlertView showAlert:str];
    
    }
    
    NSMutableArray *viewControllers = [NSMutableArray arrayWithArray:_mainVC.navigationController.viewControllers];
    UGChatViewController *chatViewContrller = nil;
    for (id viewController in viewControllers)
    {
        if ([viewController isKindOfClass:[UGChatViewController class]] && [aGroup.groupId isEqualToString:[(UGChatViewController *)viewController conversation].conversationId])
        {
...
- (void)didReceiveDeletedFromUsername:(NSString *)aUsername
{
    ...
    for (id viewController in viewControllers)
    {
        if ([viewController isKindOfClass:[UGChatViewController class]] && [aUsername isEqualToString:[(UGChatViewController *)viewController conversation].conversationId])
        {
            chatViewContrller = viewController;
            break;
        }
...
- (void)didReceiveCallIncoming:(EMCallSession *)aSession
{
    ...
    if(_callSession){
        ...
        _callController = [[UGCallViewController alloc] initWithSession:_callSession isCaller:NO status:NSLocalizedString(@"call.finished", "Establish call finished")];
                           ...
                           error:(EMError *)aError
{
    ...
    if (aReason == EMCallEndReasonHangup) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"EMCallEndReasonHangup_comment" object:nil];
    }
                     ...
                     isVideo:(BOOL)aIsVideo
{
    ...
    if (aIsVideo) {
        ...
        [_callSession setVideoBitrate:1000];
    }
    else{
    ...
    if(_callSession){
        ...
        _callController = [[UGCallViewController alloc] initWithSession:_callSession isCaller:YES status:NSLocalizedString(@"call.connecting", @"Connecting...")];
        //        _callController.modalPresentationStyle = UIModalPresentationOverFullScreen;
...
- (UGChatViewController*)_getCurrentChatView
{
    NSMutableArray *viewControllers = [NSMutableArray arrayWithArray:_mainVC.navigationController.viewControllers];
    UGChatViewController *chatViewContrller = nil;
    for (id viewController in viewControllers)
    {
        if ([viewController isKindOfClass:[UGChatViewController class]])
        {
            chatViewContrller = viewController;
            break;
        }
